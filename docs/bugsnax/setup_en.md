# Bugsnax Randomizer Setup Guide

## Required Software

- Bugsnax (Steam build ID 24392074)
- The Archipelago client

## Installation

1. Download `bugsnax.apworld` and place it in your Archipelago `custom_worlds` folder.
2. Generate or join a multiworld using a Bugsnax YAML.
3. Run the Bugsnax client via the Archipelago app while Bugsnax is running.

## Notes

- Run the client as Administrator (required for memory access).
- The randomiser is locked to save slot 2 -- start a brand new game there before connecting. Slots 1, 3, and 4 are never touched.
- The goal is beating the game (reaching the credits) AND collecting your configured (in yaml settings) Golden Snax count.
