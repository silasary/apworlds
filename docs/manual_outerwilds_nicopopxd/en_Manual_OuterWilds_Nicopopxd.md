...
At minimum you need to have both seen and received the `Warp Core Manual` and `Coordinates` and most [Progressions](#progression-items)  
And then Grab the Adv. Warp Core in game get to the Vessel and Warp to the eye.  
For more info on the location of the goal: check [Standard in Base Game or Eye requirement](#standard-in-base-game-or-eye-requirement)
...
For more info on the location of the goal: check [Standard in DLC or Prisoner Requirements](#standard-in-dlc-or-prisoner-requirements)
...
For more info on the location of the goal: check [Visit All Archives Requirement](#visit-all-archives-requirement)
...
For more info on the location of the goal: check [Ash Twin Project Break Spacetime Requirement](#ash-twin-project-break-spacetime-requirement)
...
For more info on the location of the goal: check [High Energy Lab Break Spacetime Requirement](#high-energy-lab-break-spacetime-requirement)
...
For more info on the location of the goal: check [Stuck with Solanum Requirement](#stuck-with-solanum-requirement)
...
For more info on the location of the goal: check [Stuck on the Stranger Requirement](#stuck-on-the-stranger-requirement)
...
For more info on the location of the goal: check [Stuck in the Dream Requirement](#stuck-in-the-dream-requirement)
...
New in 7.0.0, If you really want to remove items/location from the generation there are new hidden options in the Weighted Options [here (WebHost)](../player-options) to do that

## How do I install the mod for a Manual game?

There is no mandatory mods for this game.  
For more info look at the [setup guide (Webhost)](/tutorial/Manual_OuterWilds_Nicopopxd/setup/en) /
...
If you don't know what a location requires you can do `/explain [Location]` in your client and it will tell you every items you are missing
If you see an item required that starts with `[Event] event name` that mean it require going to a location that you can then do `/explain event name` to know how to get
...
- "Nomai Warp Codes": unlock using teleporters on ashtwin (you can allways use the white hole station).
...
- "Boat Patch": unlock the ability to use the loading glitch to get to the lake dream.
- "Limbo Patch": unlock the ability to use the Render Glitch to see and use the invisible platforms.
- "Alarm Bypass Patch": unlock the ability to die and thus bypass the statues that would wake you up.
- "Totem Hands Patch": unlock the usage of the hand statues in the dream to get across gap.
- "Door Candles Patch": unlock the ability to blow out the candles in the dream to open doors.
- "Ghost Wall Patch": allow you to go through the fake fireplace and walls in the dreamworld.
- "Progressive Secret Room": unlocks going in the burning rooms, the order is from the main entrance follow the water.
- "Datamined Code": replace the last 2 glitch to open the vault, counts from left to right with the vault behind you, first one is useless.
...
- "Vanilla%"
  ...
  - requires: Being able to get to the vessel, having seen the coord, and having both the adv. warp core and the coords
    - aka [5 - Ship] and [4 - Display] and |@Base Need For Eye:2|
      - [5 - Ship] aka the Vessel = |Signaloscope| and |Signal > Distress| and |Scout| and |Scout Photos| and |Anglerfish Biology|
      - [4 - Display] aka See the Coords = |Counter-Clockwise Tornadoes| and |Jellyfishes|
      - |@Base Need For Eye:2| = |Coordinates| and |Warp Core Manual|
...
- "Prisoner%"
  ...
  - requires: Getting in the vault
    - AKA (|Datamined Code:3| and |Limbo Patch|)
    - or (|Datamined Code:2| and |Limbo Patch| and |Alarm Bypass Patch|)
    - or (|Boat Patch| and |Limbo Patch| and |Alarm Bypass Patch|)
...
- "Quantum_Stuck%"
  ...
  - requires: Warp Core Manual, being able to get it and what's needed to get to solanum
    - aka "|Warp Core Manual| and [1 - Visit] and |Quantum Rule > A| and |Scout| and |Photos| and |Quantum Rule > B| and |Quantum Rule > C|"
...
- "Stranger_Stuck%"
  ...
  - requires: Warp Core Manual, being able to get it and being able to land in the stranger
    - aka "|Warp Core Manual| and [1 - Visit] and [8 - Ship]"
...
- "Quantum_Stuck%"
  ...
  - requires: Adv. warp core, being able to get it, being able to land in the stranger and any way to get in the dream
    - aka "|Warp Core Manual| and [1 - Visit] and [8 - Ship] and ([9A - Sleep] or [9B - Sleep] or [9C - Sleep] or [9D - Sleep])"
