# Item Categories

As of Haste AP v0.4.0, Persistent Items have been added, granting the player a number of items at the start of each Shard. To avoid having 100 unique items in the pool, the items have been broken down by rarity/function into the following categories:
- Common Speed (9)
- Common Support (11)
- Common Health (15)
- Rare Speed (14)
- Rare Support (7)
- Rare Health (12)
- Epic Speed (10)
- Epic Support (6)
- Epic Health (7)
- Legendary (11)

Some items have multiple purposes, and therefore fall into multiple categories. Legendary items are bundled into a single category for simplicity due to their low total counts.

Items may be recategorized if there is enough demand for it.

Items with a ***bolded italic name*** are Active Items, and can be excluded in the YAML settings.

## Speed Items
Includes items that grants Boost, Speed, spawns a Boost Ring, or grants a special movement option.

| Item Name      | Rarity |
| ----------- | ----------- |
| Golden Necklace | Common |
| Growth Potential | Common |
| ***Mysterious Spring*** | Common |
| ***Recyclable Rocket*** | Common |
| ***Rocket Boots*** | Common |
| Shortcut | Common |
| Spark Plug | Common |
| Spark Powered Propeller | Common |
| Transition Slingshot | Common |
| ***Boost Remote*** | Rare |
| Cherry on Top | Rare |
| Emergency Shoes | Rare |
| ***Energy Lash*** | Rare |
| Greed Machine | Rare |
| Heir's Determination | Rare |
| ***Personal Gravity Enhancer*** | Rare |
| Risk and Reward | Rare |
| Secret Technique Instructions | Rare |
| Shiny Anchor Pin | Rare |
| ***Standard Redirector*** | Rare |
| ***Time Dilation Thing*** | Rare |
| ***Timeline Shifter*** | Rare |
| Vitamins | Rare |
| BOOSTR POG | Epic |
| Bootleg Pattern | Epic |
| Charge Boots | Epic |
| Fragile Confidence | Epic |
| Painful Coil | Epic |
| Pathfinder | Epic |
| Perpetual Motion Machine | Epic |
| Ring Materializer | Epic |
| ***Spark Dasher*** | Epic |
| Well Earned Confidence | Epic |
| ***Blood Engine*** | Legendary* |
| ***Experimental Autopilot*** | Legendary* |
| Experimental Thrusters | Legendary* |
| Timeline Attractor | Legendary* |
| Wingspan | Legendary* |

> \*Legendary items are included in this list for documentation's sake. In-game, the Legendary items are not seperated by function.

## Support Items
Includes items grants Energy, Sparks, Luck, Cooldown reduction, Item retriggers, or Perfect landings.

| Item Name      | Rarity |
| ----------- | ----------- |
| Adrenaline | Common |
| Big Spark Magnet | Common |
| Clown Shoes | Common |
| Dynamo Treadmill | Common |
| Heart Shaped Mirror | Common |
| Interest | Common |
| Shimmering Condenser | Common |
| Spark Furnace | Common |
| Steady Investment | Common |
| Void Charger | Common |
| Void Compressor | Common |
| Greed Machine | Rare |
| High Risk Investment | Rare |
| Instant Compensation Machine | Rare |
| Jackpot | Rare |
| Karma | Rare |
| Momentum Recalibrator | Rare |
| N-Dimensional-lead Clover | Rare |
| Atomic Timepiece | Epic |
| Overwound Pocketwatch | Epic |
| Planar Reconfiguration | Epic |
| Plutonium Coin | Epic |
| Tight Schedule | Epic |
| Timeline Recalibrator | Epic |
| Dangerous Investment Scheme | Legendary* |
| Flashback | Legendary* |
| General Relativity | Legendary* |
| Overcomplicated Coin | Legendary* |
| ***Portable Harvester*** | Legendary* |

> \*Legendary items are included in this list for documentation's sake. In-game, the Legendary items are not seperated by function.

## Health Items
Includes items that grants Healing, increases Max Health, or grants Invulnerability.

| Item Name      | Rarity |
| ----------- | ----------- |
| Aromatic Herbs | Common |
| Big Pumpkin | Common |
| Bitter Herbs | Common |
| Delayed Emergency Device | Common |
| Distance-Based Health Insurance | Common |
| Grunt's Helmet | Common |
| Mortar and Pestle | Common |
| Overclocked Medical Drone | Common |
| Performance Based Health Insurance | Common |
| Pocket Snack | Common |
| Protective Medallion | Common |
| Reheated Soup | Common |
| ***Replenishing Vial*** | Common |
| Restorative Maneuver | Common |
| Velocity Powered Syringe | Common |
| Big Squash | Rare |
| Brittle Breastplate | Rare |
| Emergency Shoes | Rare |
| Energy Funnel | Rare |
| Fragile Taco | Rare |
| Greed Machine | Rare |
| Impulse Actived Stabilizer | Rare |
| Intangibility | Rare |
| Leadership Pipe | Rare |
| Low Grade Timeline Swapper | Rare |
| ***Personal Matter Stabilizer*** | Rare |
| Quick Taco | Rare |
| Speedy Recovery | Rare |
| Extreme Herbs | Epic |
| Friendly Looking Star | Epic |
| Impact Activated Healing Drone | Epic |
| Pungent Herbs | Epic |
| ***Steel Hat Lining*** | Epic |
| Timeline Refactor | Epic |
| ***Otherworldly Contact*** | Legendary* |

> \*Legendary items are included in this list for documentation's sake. In-game, the Legendary items are not seperated by function.
